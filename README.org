...
of your code, and serialization goes back to text the same way it came in. That's why we have the ~myvaluesholder~
in the above code example. If we simply called ~toString~ on ~myvalues~ we would get
